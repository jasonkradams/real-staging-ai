...
- Added idempotency scaffolding in the handler (duplicate event short-circuit) with TODOs to back it via a processed_events table.
...
- Added repositories for processed events and subscriptions; wired idempotency to use the processed events repository from the webhook handler.
- Wired subscription persistence in handlers (created/updated/deleted) and linked stripe_customer_id to the user on checkout.session.completed when client_reference_id is present.
- Next steps to complete Stripe:
  - Verify subscription field mapping coverage (status, price, current_period_start/end, cancel flags) and add tests.
  - Add optional invoices persistence and repositories (if we want invoice-level records).
...
- API (Go + Echo)
  ...
  - Stripe: Webhook endpoint is present and routes events to typed handlers. Signature verification and persistence logic are not implemented yet.
...
- Stripe
  - [ ] Add verifyStripeSignature helper with tolerance and constant-time compare
  - [ ] Add processed_events table and repository
  - [ ] Add subscriptions (and optional invoices) tables + repositories
  - [ ] Implement webhook handlers to persist user/subscription/invoice data
  - [ ] Unit tests for signature + handlers; env-gated integration tests
  - [ ] Require STRIPE_WEBHOOK_SECRET in prod
- SSE
...
- Cleanup/Infra/Docs
  ...
  - [ ] Choose license
  - [ ] CI: lint/test; add preview environment
