...
- Job Queue (P0)
  - Replace mock queue client with production-ready Redis/asynq client in worker and API.
  - Ensure API enqueues `stage:run` with the correct payload on image creation.
  - Persist job metadata (if desired) for observability; otherwise rely on image status + SSE for Phase 1.
...
P0
- [ ] Replace mock queue with Redis/asynq in API and worker; define task payload contract.
