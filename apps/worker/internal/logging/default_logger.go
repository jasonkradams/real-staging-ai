...
// NewDefaultLogger constructs a slog-backed Logger with JSON output and worker service fields.
func NewDefaultLogger() Logger {
	lvl := parseLevel(os.Getenv("LOG_LEVEL"))
	h := slog.NewJSONHandler(os.Stdout, &slog.HandlerOptions{Level: lvl})
	l := slog.New(h).With("service", "virtual-staging-worker")
	return &defaultSLogger{l: l}
}

// NewDefault returns a fresh slog-based logger instance.
func NewDefault() Logger { return NewDefaultLogger() }
