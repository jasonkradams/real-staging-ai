"use client";

import { useState } from "react";
import { apiFetch } from "@/lib/api";

export default function UploadPage() {
  const [file, setFile] = useState<File | null>(null);
  const [projectId, setProjectId] = useState("b0eebc99-9c0b-4ef8-bb6d-6bb9bd380a12");
  const [status, setStatus] = useState<string>("");
  const [imageId, setImageId] = useState<string>("");

  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    setStatus("");
    setImageId("");

    if (!file) {
      setStatus("Please select a file.");
      return;
    }

    try {
      // 1) Presign
      setStatus("Presigning upload URL...");
      const presign = await apiFetch<{ upload_url: string; file_key: string }>(
        "/v1/uploads/presign",
        {
          method: "POST",
          body: JSON.stringify({
            filename: file.name,
            content_type: file.type || "application/octet-stream",
            file_size: file.size,
          }),
        }
      );

      // 2) Upload to S3 via presigned URL
      setStatus("Uploading to S3...");
      const putRes = await fetch(presign.upload_url, {
        method: "PUT",
        headers: {
          "Content-Type": file.type || "application/octet-stream",
        },
        body: file,
      });
      if (!putRes.ok) {
        throw new Error(`Upload failed: ${putRes.status}`);
      }

      // 3) Create Image with original_url pointing to uploaded key
      // Note: The API expects a public URL shape; for dev/local, the backend translates/stages.
      const originalUrl = `https://test-bucket.s3.amazonaws.com/${presign.file_key}`;

      setStatus("Creating image job...");
      const created = await apiFetch<{ id: string }>("/v1/images", {
        method: "POST",
        body: JSON.stringify({ project_id: projectId, original_url: originalUrl }),
      });

      setImageId(created.id);
      setStatus("Image created! Use the Images page to watch status via SSE.");
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      setStatus(message);
    }
  }

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold">Upload</h1>
      <form onSubmit={onSubmit} className="card">
        <div className="card-header">Presign & Upload</div>
        <div className="card-body space-y-4">
          <div>
            <label className="block text-sm mb-1">Project ID</label>
            <input
              className="input"
              value={projectId}
              onChange={(e) => setProjectId(e.target.value)}
              placeholder="project UUID"
            />
          </div>
          <div>
            <label className="block text-sm mb-1">File</label>
            <input
              type="file"
              onChange={(e) => setFile(e.target.files?.[0] || null)}
              className="block w-full"
            />
          </div>
          <div className="flex items-center gap-2">
            <button className="btn" type="submit">
              Upload & Create Image
            </button>
            {status && <span className="text-sm text-gray-600">{status}</span>}
          </div>
        </div>
      </form>

      {imageId && (
        <div className="card">
          <div className="card-header">Created</div>
          <div className="card-body">
            <div className="text-sm text-gray-600">
              Image ID: <code className="px-1 py-0.5 bg-gray-100 rounded">{imageId}</code>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}
