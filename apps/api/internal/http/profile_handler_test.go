...
func TestNewProfileHandler(t *testing.T) {
	t.Run("success: create new profile handler", func(t *testing.T) {
		...
		handler := NewProfileHandler(service, log)
...
func TestProfileHandler_GetProfile(t *testing.T) {
	testCases := []struct {
		name           string
		auth0Sub       string
		setupMock      func(*user.ProfileServiceMock)
		expectedStatus int
	...
	}{
		{
			...
			setupMock: func(service *user.ProfileServiceMock) {
		...
		},
		{
			...
			setupMock: func(service *user.ProfileServiceMock) {
				service.GetProfileByAuth0SubFunc = func(ctx context.Context, auth0Sub string) (*user.ProfileResponse, error) {
					...
					assert.Equal(t, "auth0|testuser", auth0Sub)
...
func TestProfileHandler_UpdateProfile(t *testing.T) {
	testCases := []struct {
		...
		{
			name:     "fail: cannot get current profile",
			auth0Sub: "auth0|12345",
			requestBody: map[string]interface{}{
				"email": "test@example.com",
			},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Set Auth0 subject in context if provided
			if tc.auth0Sub != "" {
				c.Set("user_id", tc.auth0Sub)
			}
