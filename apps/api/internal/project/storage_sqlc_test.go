...
func TestProjectStorageSQLc_CreateProject(t *testing.T) {
	...
	storage := project.NewStorageSQLc(db)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			createdProject, err := storage.CreateProject(ctx, tc.project, tc.userID)
...
func TestProjectStorageSQLc_GetProjectsByUserID(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			projects, err := storage.GetProjectsByUserID(ctx, tc.userID)
...
func TestProjectStorageSQLc_GetProjectByID(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			project, err := storage.GetProjectByID(ctx, tc.projectID)
...
func TestProjectStorageSQLc_UpdateProject(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			updatedProject, err := storage.UpdateProject(ctx, tc.projectID, tc.newName)
...
func TestProjectStorageSQLc_UpdateProjectByUserID(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			updatedProject, err := storage.UpdateProjectByUserID(ctx, tc.projectID, tc.userID, tc.newName)
...
func TestProjectStorageSQLc_DeleteProject(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := storage.DeleteProject(ctx, tc.projectID)
			...
			if tc.projectID == "b0eebc99-9c0b-4ef8-bb6d-6bb9bd380a12" {
				_, err := storage.GetProjectByID(ctx, tc.projectID)
...
func TestProjectStorageSQLc_DeleteProjectByUserID(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := storage.DeleteProjectByUserID(ctx, tc.projectID, tc.userID)
...
func TestProjectStorageSQLc_CountProjectsByUserID(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			count, err := storage.CountProjectsByUserID(ctx, tc.userID)
...
func TestProjectStorageSQLc_Integration(t *testing.T) {
	...
	createdProject, err := storage.CreateProject(ctx, newProject, userID)
	require.NoError(t, err)
	assert.Equal(t, "Integration Test Project", createdProject.Name)

	// 2. Get projects by user ID
	projects, err := storage.GetProjectsByUserID(ctx, userID)
	require.NoError(t, err)
	assert.Len(t, projects, 2) // seed project + new project

	// 3. Count projects
	count, err := storage.CountProjectsByUserID(ctx, userID)
	require.NoError(t, err)
	assert.Equal(t, int64(2), count)

	// 4. Update the project
	updatedProject, err := storage.UpdateProjectByUserID(ctx, createdProject.ID, userID, "Updated Integration Project")
	require.NoError(t, err)
	assert.Equal(t, "Updated Integration Project", updatedProject.Name)

	// 5. Get the project by ID
	retrievedProject, err := storage.GetProjectByID(ctx, createdProject.ID)
	require.NoError(t, err)
	assert.Equal(t, "Updated Integration Project", retrievedProject.Name)

	// 6. Delete the project
	err = storage.DeleteProjectByUserID(ctx, createdProject.ID, userID)
	require.NoError(t, err)

	// 7. Verify it's deleted
	_, err = storage.GetProjectByID(ctx, createdProject.ID)
	assert.Error(t, err) // Should not be found
}
